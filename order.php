...
<?php
//cheak weather the food id is set or not
if (isset($_GET['food_id'])) {
    //get the food id and detail of selected food
    $food_id = $_GET['food_id'];

    //get the detail of selected food
    $sql = "SELECT * FROM tbl_food WHERE id=$food_id";
    //exicute the query
    $res = mysqli_query($conn, $sql);
    //count the rows
    $count = mysqli_num_rows($res);
    //cheak weather the data is available or not
    if ($count == 1) {
        //we hava data 
...
<section class="food-search">
    <div class="container">
        ...
        <form action="" method="POST" class="order">
            <fieldset>
                ...
                <div class="food-menu-img">
                    <?php
                    //chek weather image name is available or not 
                    if ($image_name == "") {
                        //image not available
            ...
            <fieldset>
                ...
                <input type="text" name="full-name" placeholder="E.g. Sumit Satyam" class="input-responsive" required>
        ...
        if (isset($_POST['submit'])) {
            ...
            //cheak weather query exicuted sucessfully or not
            if ($res2 == true) {
                //query exicute and order set
                $_SESSION['order'] = "<div class='sucess text-center'>Order Placed.</div>";
                header('location:' . SITEURL);
            } else {
                //fail to save order
                $_SESSION['order'] = "<div class='error text-center'>Fail To Placed Order.</div>";
                header('location:' . SITEURL);
            }
        }
